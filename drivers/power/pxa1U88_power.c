...
#define APMU_TRACE		APMU_REG(0x108)
...
#define DBGCLK_EN		(1 << 3)
#define TRACE_CLK_EN	(1 << 4)
void enable_trace(int on)
{
	if (on) {
		CLK_SET_BITS(APMU_TRACE, (DBGCLK_EN | TRACE_CLK_EN), 0);
	} else {
		CLK_SET_BITS(APMU_TRACE, 0, (DBGCLK_EN | TRACE_CLK_EN));
	}
}
