...
static struct lcd_videomode video_wxga_mode_p = { //noise
	.name = "WXGA",
	.refresh = 60,
	.xres = 800,
	.yres = 1280,
...
static struct lcd_videomode video_wxga_mode_p_spi_micom = {
	.name = "WXGA",
	.refresh = 60,
	.xres = 800,
	.yres = 1280,
...
static struct mmp_disp_plat_info mmp_mipi_lcd_info_p_spi_micom = {
	.index = 0,
	.id = "GFX Layer",
	/* FIXME */
	.sclk_src = 416000000,
	.sclk_div = 0x20001106,
	.num_modes = 1,
	.modes = &video_wxga_mode_p_spi_micom,
	.pix_fmt = PIX_FMT_RGBA888,
	.burst_len = 16,
	/*
	 * don't care about io_pin_allocation_mode and dumb_mode
...
static void panel_wxga_init_config(struct mmp_disp_info *fbi,int en)
{
	if(en) {
		vx5b1d_reset();
		if (0 != vx5b1d_init(get_lcd_landscape(),get_lcd_power(),get_board_spi(),get_board_micom()))
			return NULL;
	}
}
...
static void *lcd_init(void)
{
	...
	{
		if(get_board_spi() || get_board_micom())
		{
			mi = &mmp_mipi_lcd_info_p_spi_micom;
		}
		else
		{
...
void handle_dtb(struct fdt_header *devtree)
{
	...
	else if(get_board_spi() || get_board_micom()) {
		if(handle_disp_rsvmem(&mmp_mipi_lcd_info_p_spi_micom))
		printf("Attention, framebuffer maybe not reserved correctly!\n");
	}
	...
	if(get_board_spi())
	{
		run_command("fdt set /panel_vx5b1d vx5b1d,spi <1>", 0);
	}
}
#endif /* CONFIG_OF_LIBFDT */
...
void board_pmic_power_fixup(struct pmic *p_power)
{
	...
	marvell88pm_set_ldo_vol(p_power, 11, 3300000);	/* -> TP+3.3V */
