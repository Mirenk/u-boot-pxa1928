...
static const uint32_t vx5b1d_reg_table_p[] =
{//noise
0x700, 0x34900040,
0x704, 0x1039A,
0x70C, 0x00004600,
...
static const uint32_t vx5b1d_reg_table_p_micom[] =
{
0x700 , 0x34900040 ,
0x704 , 0x10310 ,
0x70C , 0x00004600 ,
 ...
 *
 ******************************************************************************/
int vx5b1d_init(int landscape,int power,int spi,int micom)
{
	int i;
	uint32_t ready;
	...
	else
	{
		if(spi) {
			...
			if(micom)
			{
				for (i=0; i<ARRAY_SIZE(vx5b1d_reg_table_p_micom); i+=2) {
					int ret;
					uint32_t addr = vx5b1d_reg_table_p_micom[i+0];
					uint32_t data = vx5b1d_reg_table_p_micom[i+1];
					ret = vx5b1d_reg_write32(addr, data);
					if (ret != 0) {
						printf(TAG ": cannot write register. (seq=%d)\n", i);
						return -1;
				...
				for (i=0; i<ARRAY_SIZE(vx5b1d_reg_table_p); i+=2) {
					int ret;
					uint32_t addr = vx5b1d_reg_table_p[i+0];
					uint32_t data = vx5b1d_reg_table_p[i+1];
					ret = vx5b1d_reg_write32(addr, data);
					if (ret != 0) {
						printf(TAG ": cannot write register. (seq=%d)\n", i);
						return -1;
